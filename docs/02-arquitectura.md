...
| ADR-6 | Autenticación JWT propia con soporte MFA TOTP en vez de Entra ID en el MVP | Entra ID/Entra External ID requiere tenant real que no existe en este entorno de build; se deja el `IIdentityProvider` documentado para migrar sin tocar el dominio. |
